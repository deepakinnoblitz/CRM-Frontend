...
export function EmployeeEvaluationEventDetailsDialog({ open, onClose, event }: Props) {
  ...
  useEffect(() => {
    if (open && event?.employee) {
      getEmployee(event.employee).then(setEmployeeDetails).catch(console.error);
    } else {
  ...
  const statusConf = STATUS_CONFIG[docstatus.toString()] || {
    color: '#64748b',
    bg: '#f1f5f9',
    border: '#e2e8f0',
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3, pl: 1 }}>
              ...
              <InfoRow
                ...
                value={fDate(evaluation_date, 'DD-MM-YYYY')}
              />
            </Box>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2, pl: 1 }}>
              <Box>
                ...
                <Label
                  variant="soft"
                  color={
                    (['Excellent', 'Good', 'Positive', 'Agree'].includes(evaluation_type) &&
                      'success') ||
                    (['Average', 'Neutral'].includes(evaluation_type) && 'warning') ||
                    (['Bad', 'Very Bad', 'Negative', 'Disagree'].includes(evaluation_type) &&
                      'error') ||
                    'default'
                  }
            ...
            <Box sx={{ pl: 1, pr: 2 }}>
              ...
              >
                <Typography
                  variant="body2"
                  sx={{ color: 'text.primary', lineHeight: 1.6, fontWeight: 600 }}
                >
