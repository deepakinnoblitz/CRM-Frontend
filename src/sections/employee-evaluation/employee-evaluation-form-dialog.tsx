...
import {
  createEmployeeEvaluationEvent,
  updateEmployeeEvaluationEvent,
...
export function EmployeeEvaluationEventFormDialog({
  open,
  onClose,
  onSuccess,
  selectedEvent,
  ...
  useEffect(() => {
    if (open) {
      ...
      getForValueOptions('Evaluation Trait').then(setTraits).catch(console.error);
    }
  }, [open]);
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            <DatePicker
              ...
              onChange={(newValue) =>
                setFormData({ ...formData, evaluation_date: newValue?.format('YYYY-MM-DD') || '' })
              }
              slotProps={{
                textField: {
                  fullWidth: true,
