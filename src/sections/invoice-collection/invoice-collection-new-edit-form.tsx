...
import { createInvoiceCollection, updateInvoiceCollection } from 'src/api/invoice-collection';
...
const InvoiceCollectionNewEditForm = forwardRef(
  ({ currentInvoiceCollection, onLoadingChange }: Props, ref) => {
    ...
    useEffect(() => {
      getDoctypeList('Invoice', ['name', 'client_name', 'customer_name', 'grand_total']).then(
        (data) => {
          ...
          >
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                typeof option === 'string' ? option : `${option.name} - ${option.customer_name}`
              }
            ...
            <DatePicker
              ...
              onChange={(newValue) =>
                handleChange('collection_date', newValue?.format('YYYY-MM-DD') || '')
              }
              slotProps={{
                textField: {
                  fullWidth: true,
