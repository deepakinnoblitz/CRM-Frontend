...
export function WhatsappAutomationDialog({
  ...
  useEffect(() => {
    ...
    frappeRequest(
      `/api/method/company.company.doctype.crm_whatsapp_automation.crm_whatsapp_automation.get_automation_preview?doctype=Lead&docname=${encodeURIComponent(lead.name)}&proposal_name=${encodeURIComponent(selectedProposal)}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ fontWeight: 800, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Iconify icon={"ic:baseline-whatsapp" as any} width={28} sx={{ color: '#25D366' }} />
        {automation.title || 'Send WhatsApp Message'}
      </DialogTitle>
