...
export function AttendanceView() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (existing.data.length > 0 && existing.data[0].name !== currentAttendanceId) {
        setSnackbar({
          open: true,
          message: `Attendance already marked for ${formData.employee} on ${formData.attendance_date}`,
          severity: 'error',
        });
        setCreating(false);
        return;
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} records deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      await refetch();
  ...
  const renderField = (
    ...
    if (type === 'hidden' || extraProps?.hidden) {
      return <input type="hidden" name={fieldname} value={formData[fieldname] || ''} />;
    }
    const commonProps = {
      fullWidth: true,
      label,
    ...
    <DashboardContent maxWidth={false}>
      ...
      <Card>
        <AttendanceTableToolbar
          ...
          canReset={
            !!startDate || !!endDate || !!filterName || filterStatus !== 'all' || !!filterEmployee
          }
        />
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Box display="grid" margin={2} gridTemplateColumns="1fr" gap={3}>
              ...
              {renderField(
                ...
                'Status',
                'select',
                ['Present', 'Absent', 'Half Day', 'On Leave', 'Holiday', 'Missing'],
                { hidden: false }
              )}
              ...
              <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr' }} gap={2}>
                {renderField('working_hours_display', 'Working Hours', 'text', [], {
                  InputProps: { readOnly: true },
                })}
                {renderField('overtime_display', 'Overtime Hours', 'text', [], {
                  InputProps: { readOnly: true },
                })}
              </Box>
      ...
      <AttendanceImportDialog open={openImport} onClose={handleCloseImport} onRefresh={refetch} />
    </DashboardContent>
  );
}
