...
import {
  ...
  updateHolidayList,
  deleteHolidayList,
  getHolidayListPermissions,
} from 'src/api/holiday-lists';
...
export function HolidaysView() {
  ...
  const { data, total, refetch } = useHolidayLists(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} holiday list(s) deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      refetch();
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to delete holiday lists',
        severity: 'error',
      });
    }
  };
  ...
  const handleEditRow = useCallback(async (row: any) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to load holiday list',
        severity: 'error',
      });
    }
  }, []);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to delete holiday list',
        severity: 'error',
      });
    } finally {
            ...
            >
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (formErrors.holidayListName)
                    setFormErrors((prev) => ({ ...prev, holidayListName: '' }));
                }}
                required
                        ...
                        >
                          ...
                          <TableCell sx={{ py: 1.5 }}>
                            <TextField
                              ...
                              onChange={(e) =>
                                handleHolidayChange(index, 'description', e.target.value)
                              }
