...
export function LeadReportView() {
  ...
  const handleExport = async (selectedFields: string[], format: 'excel' | 'csv') => {
    ...
    try {
      ...
      const res = await fetch(`/api/method/frappe.client.get_list?${query.toString()}`, {
        credentials: 'include',
      });
      ...
      <Stack spacing={3}>
        ...
        >
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            <Select value={leadsType} onChange={(e) => setLeadsType(e.target.value)} displayEmpty>
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            <Select value={leadsFrom} onChange={(e) => setLeadsFrom(e.target.value)} displayEmpty>
        ...
        >
          ...
          <SummaryCard
            item={{ label: 'Incoming Leads', value: incomingLeads, indicator: 'green' }}
          />
          <SummaryCard
            item={{ label: 'Outgoing Leads', value: outgoingLeads, indicator: 'orange' }}
          />
        </Box>
                    ...
                    .map((row, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Box
                              sx={{
                                ...
                                fontWeight: 'bold',
                                bgcolor: alpha(
                                  row.leads_type === 'Incoming' ? '#4CAF50' : '#FF9800',
                                  0.1
                                ),
                          ...
                          >
                            <IconButton
                              onClick={() => handleViewLead(row.name)}
                              sx={{ color: 'info.main' }}
                            >
