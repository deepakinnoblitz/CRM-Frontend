          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2.5 }}>
              ...
              <Box sx={{ flexGrow: 1 }}>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary', fontWeight: 600 }}>
                  {meeting.meet_for} with{' '}
                  {meeting.meet_for === 'Lead'
                    ? meeting.lead_name
                    : meeting.meet_for === 'Contact'
                      ? meeting.contact_name
                      : meeting.meet_for === 'Others'
                        ? meeting.enter_id
                        : meeting.accounts_name || '-'}
                </Typography>
              </Box>
            ...
            <Box>
              ...
              >
                ...
                <DetailItem
                  ...
                  value={meeting.completed_meet_status}
                  icon="solar:check-read-bold"
                />
                <DetailItem
                  label="From"
                  value={meeting.from ? new Date(meeting.from).toLocaleString() : '-'}
                  icon="solar:clock-circle-bold"
                />
                <DetailItem
                  label="To"
                  value={meeting.to ? new Date(meeting.to).toLocaleString() : '-'}
                  icon="solar:clock-circle-bold"
                />
                <DetailItem
                  label="Venue"
                ...
                <DetailItem
                  label="Participants"
                  value={meeting.participants?.map((p: any) => p.user).join(', ')}
            ...
            <Box sx={{ p: 3, bgcolor: 'background.neutral', borderRadius: 2 }}>
              ...
              >
                <DetailItem
                  label="Created On"
                  value={new Date(meeting.creation).toLocaleString()}
                  icon="solar:calendar-date-bold"
                />
                <DetailItem
                  label="Modified On"
                  value={new Date(meeting.modified).toLocaleString()}
