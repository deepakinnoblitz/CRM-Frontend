  ...
  const renderDetails = (
    ...
    >
      ...
      <DetailItem
        ...
        label="Closes On"
        value={formatDate(job.closes_on)}
      />
      ...
      <DetailItem
        ...
        value={
          job.lower_range
            ? `₹${job.lower_range} - ₹${job.upper_range} per ${job.salary_per}`
            : 'Not Disclosed'
        }
      />
      <DetailItem
  ...
  const renderDescription = (
    <Stack spacing={3} sx={{ mt: 2, mb: 5, ml: 2 }}>
      ...
      <Box>
        ...
        <Box sx={{ mt: 2, color: 'text.secondary', whiteSpace: 'pre-wrap' }}>
          <Typography variant="body2">{job.description || 'No description provided.'}</Typography>
        </Box>
      </Box>
    </Stack>
  );
