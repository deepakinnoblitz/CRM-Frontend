...
import {
  ...
  getReimbursementClaimWorkflowActions,
...
export function ReimbursementClaimDetailsDialog({ open, onClose, claim, onRefresh }: Props) {
  ...
  const fetchActions = async () => {
    try {
      const workflowActions = await getReimbursementClaimWorkflowActions(
        claim.workflow_state || 'Draft'
      );
      // Filter out Pay and Mark Paid actions
      const filteredActions = workflowActions.filter(
        (action) => !['Pay', 'Mark Paid'].includes(action.action)
      );
  ...
  const handleApplyAction = async () => {
    ...
    try {
      ...
      await applyReimbursementClaimWorkflowAction(
        claim.name,
              ...
              <Box>
                ...
                >
                  ...
                  <DetailItem
                    ...
                    value={dayjs(claim.date_of_expense).format('DD/MM/YYYY')}
                    icon="solar:calendar-bold"
                  />
                  <DetailItem
                    label="Amount"
                    value={`₹${claim.amount?.toLocaleString() || 0}`}
                    icon="solar:wad-of-money-bold"
                  />
                  <DetailItem
                    label="Status"
                    value={claim.paid === 1 ? 'Paid' : 'Pending'}
              ...
              {(claim.workflow_state === 'Paid' || claim.paid === 1) && (
                <Box sx={{ pt: 3 }}>
                  ...
                  >
                    <DetailItem
                      label="Approved By"
                      value={claim.approved_by || '-'}
                      icon="solar:user-bold"
                    />
                    <DetailItem
                      label="Status"
                      value={claim.workflow_state || (claim.paid === 1 ? 'Paid' : 'Pending')}
                    ...
                    <DetailItem
                      label="Paid Date"
                      value={claim.paid_date ? dayjs(claim.paid_date).format('DD/MM/YYYY') : '-'}
                      icon="solar:calendar-bold"
                    />
                    <DetailItem
                ...
                >
                  <Box>
                    ...
                    <Typography variant="body2">
                      {dayjs(claim.creation).format('DD/MM/YYYY HH:mm')}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography
                    ...
                    <Typography variant="body2">
                      {dayjs(claim.modified).format('DD/MM/YYYY HH:mm')}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography
      ...
      <ConfirmDialog
        ...
        content={
          selectedAction?.action === 'Pay' ? (
            <Box sx={{ pt: 1 }}>
              <Typography sx={{ mb: 2 }}>
                Are you sure you want to mark this claim as <b>Paid</b>?
              </Typography>
              <Stack spacing={2}>
                <TextField
                  fullWidth
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DatePicker
                    ...
                    onChange={(newValue) =>
                      setPaymentDate(newValue ? dayjs(newValue).format('YYYY-MM-DD') : null)
                    }
            ...
            <Stack spacing={2}>
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  onChange={(newValue) =>
                    setEditPaymentDate(newValue ? dayjs(newValue).format('YYYY-MM-DD') : null)
                  }
                  slotProps={{ textField: { fullWidth: true } }}
                />
              </LocalizationProvider>
...
function getActionIcon(action: string) {
  ...
  if (['Reject', 'Rejected', 'Cancel', 'Cancelled'].includes(action))
        ...
        {action && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Divider
              ...
              flexItem
              sx={{ height: 16, my: 'auto', borderStyle: 'dashed' }}
            />
            {action}
          </Box>
        )}
