...
export function TimesheetsReportView() {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = reportData
        .filter((d) => d.timesheet_date !== 'TOTAL')
        .map((n, index) => `${n.employee}-${index}`);
    ...
    <DashboardContent maxWidth={false}>
      <Stack spacing={3}>
          ...
          <Stack direction="row" gap={1.5} alignItems="center" flexWrap="wrap">
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              <Select value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                <MenuItem value="date_asc" sx={{ fontSize: '0.85rem' }}>
                  Date ↓ (Asc)
                </MenuItem>
                <MenuItem value="date_desc" sx={{ fontSize: '0.85rem' }}>
                  Date ↑ (Desc)
                </MenuItem>
                <MenuItem value="name_asc" sx={{ fontSize: '0.85rem' }}>
                  Name: A to Z
                </MenuItem>
                <MenuItem value="name_desc" sx={{ fontSize: '0.85rem' }}>
                  Name: Z to A
                </MenuItem>
              </Select>
            </FormControl>
        ...
        >
          ...
          <SummaryCard
            item={{ label: 'Total Hours', value: totalHours, indicator: 'green', suffix: 'hrs' }}
          />
        </Box>
                ...
                <TableHead>
                  <TableRow sx={{ bgcolor: '#f4f6f8' }}>
                    <TableCell padding="checkbox">
                      <Checkbox
                        indeterminate={
                          selected.length > 0 && selected.length < reportData.length - 1
                        }
                ...
                <TableBody>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>
                            {fDate(row.timesheet_date, 'DD-MM-YYYY')}
                          </TableCell>
                          <TableCell>
                  ...
                  {reportData.find((d) => d.timesheet_date === 'TOTAL') && (
                    <TableRow sx={{ bgcolor: alpha(theme.palette.success.main, 0.08) }}>
                      <TableCell padding="checkbox" />
                      <TableCell colSpan={4} sx={{ fontWeight: 'bold', color: 'success.main' }}>
                        TOTAL
                      </TableCell>
    ...
    >
      <Stack direction="row" alignItems="center" spacing={2.5}>
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Typography
            variant="subtitle2"
            sx={{ color: 'text.secondary', fontWeight: 700, mb: 0.5 }}
          >
            {item.label}
          </Typography>
