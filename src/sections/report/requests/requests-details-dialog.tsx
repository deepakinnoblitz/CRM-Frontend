...
export function RequestDetailsDialog({ open, onClose, request, onRefresh, socket }: Props) {
  ...
  const hrQueryCount = ['hr_query', 'hr_query_2', 'hr_query_3', 'hr_query_4', 'hr_query_5'].filter(
    (f) => internalRequest?.[f]
  ).length;
  const employeeReplyCount = [
    'employee_reply',
    'employee_reply_2',
    'employee_reply_3',
    'employee_reply_4',
    'employee_reply_5',
  ].filter((f) => internalRequest?.[f]).length;
    ...
    <Box sx={{ height: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        {isEmployee &&
          ...
          internalRequest.workflow_state === 'Clarification Requested' && (
            <Typography
              variant="caption"
              color="error"
        ...
        {!isEmployee &&
          ...
          (internalRequest.workflow_state === 'Pending' ||
            internalRequest.workflow_state === 'Clarification Requested' ||
            !internalRequest.workflow_state) && (
            <Typography
              variant="caption"
              color="error"
              sx={{ textAlign: 'center', mt: 1, fontWeight: 700 }}
  ...
  return (
    ...
    >
            ...
            <Box sx={{ gridColumn: { lg: hasHistory ? 'span 8' : 'span 12' } }}>
              <Stack spacing={3}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3 }}>
                  <DetailItem
                    label="Subject"
                    value={internalRequest.subject}
                    icon="solar:document-text-bold"
                  />
                  <DetailItem
                    label="Created On"
                    value={
                      internalRequest.creation
                        ? new Date(internalRequest.creation).toLocaleString()
                        : '-'
                    }
      ...
      <ClarificationDialog
        ...
        onConfirm={(msg) =>
          handleUpdateStatus(
            clarificationType === 'HR' ? 'Clarification Requested' : 'Pending',
            msg
          )
        }
