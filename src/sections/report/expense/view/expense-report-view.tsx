...
export function ExpenseReportView() {
  ...
  const handleExport = async (selectedFields: string[], format: 'excel' | 'csv') => {
    ...
    try {
      const idsToExport = (
        selected.length > 0 ? selected : reportData.map((r) => r.expense_no)
      ).filter(Boolean);
      ...
      const fieldsToFetch =
        selectedFields.length > 0
          ? selectedFields
          : ['expense_no', 'expense_category', 'date', 'payment_type', 'total'];
        ...
        >
          ...
          {summaryData.length === 0 && (
            <>
              ...
              />
              <SummaryCard
                item={{ label: 'Total Quantity', value: 0, indicator: 'green', datatype: 'Float' }}
              />
