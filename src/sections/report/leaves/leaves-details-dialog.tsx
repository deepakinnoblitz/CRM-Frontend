...
export function LeavesDetailsDialog({ open, onClose, leaveId, onRefresh, socket }: Props) {
  ...
  const isHR =
    userRoles.some((r) => r.includes('HR')) ||
  ...
  const handleApplyAction = async (actionOverride?: WorkflowAction, message?: string) => {
    ...
    try {
      ...
      const isClarify =
        lowerAction.includes('clarification') ||
        lowerAction.includes('query') ||
        lowerAction.includes('reply');
        ...
        {isEmployee &&
          ...
          leave?.workflow_state === 'Clarification Requested' && (
            <Typography
              variant="caption"
              color="error"
              sx={{ textAlign: 'center', mt: 1, fontWeight: 700 }}
            >
        ...
        <DialogContent sx={{ p: { xs: 2.5, sm: 4 }, pt: 0 }}>
          {loading && !leave ? (
            ...
            >
              ...
              <Box sx={{ gridColumn: { lg: hasHistory ? 'span 8' : 'span 12' } }}>
                <Stack spacing={3}>
                  ...
                  >
                    ...
                    >
                      <Stack spacing={0.5}>
                        ...
                        <Typography
                          variant="subtitle2"
                          sx={{ fontWeight: 800, fontSize: { xs: 11, sm: 14 } }}
                        >
                  ...
                  >
                    <DetailRow
                      label="Applied On"
                      value={dayjs(leave?.creation).format('DD MMM YYYY HH:mm')}
                    ...
                    <DetailRow
                      ...
                      value={
                        leave?.half_day === 1
                          ? `Yes (${dayjs(leave?.half_day_date).format('DD MMM YYYY')})`
                          : 'No'
                      }
                    ...
                    <DetailRow
                      label="From Date"
                      value={dayjs(leave?.from_date).format('DD MMM YYYY')}
                      icon="solar:calendar-date-bold"
                    />
                    <DetailRow
                      label="To Date"
                      value={dayjs(leave?.to_date).format('DD MMM YYYY')}
                      icon="solar:calendar-date-bold"
                    />
                  </Box>
              ...
              {hasHistory && <Box sx={{ gridColumn: { lg: 'span 4' } }}>{renderConversation}</Box>}
            </Box>
          ) : (
            ...
            <DialogActions sx={{ p: 3, justifyContent: 'flex-end', gap: 1.5 }}>
              {filteredActions.map((action) => (
                <Button
                  ...
                  startIcon={
                    submitting && selectedAction?.action === action.action ? (
      ...
      >
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Iconify
            ...
            sx={{ color: getActionColor(selectedAction?.action || '') }}
          />
        ...
        <DialogActions>
          <Button color="inherit" onClick={() => setCommentDialogOpen(false)}>
            Cancel
          </Button>
          <Button
            variant="contained"
            ...
            disabled={submitting}
            startIcon={
              submitting ? <Iconify icon={'svg-spinners:18-dots-indicator' as any} /> : null
            }
          >
