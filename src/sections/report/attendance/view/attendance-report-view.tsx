  ...
  return (
    <DashboardContent maxWidth={false}>
      <Stack spacing={3}>
        ...
        >
          <SummaryCard
            item={{ label: 'Total Entries', value: reportData.length, indicator: 'blue' }}
          />
          <SummaryCard
            item={{
              label: 'Present',
              value: reportData.filter((d) => d.status === 'Present').length,
              indicator: 'green',
            }}
          />
          <SummaryCard
            item={{
              label: 'Absent',
              value: reportData.filter((d) => d.status === 'Absent').length,
              indicator: 'red',
            }}
          />
          <SummaryCard
            item={{
              label: 'Half Day',
              value: reportData.filter((d) => d.status === 'Half Day').length,
              indicator: 'orange',
            }}
          />
          <SummaryCard
            item={{
              label: 'Missing',
              value: reportData.filter((d) => d.status === 'Missing').length,
              indicator: 'orange',
            }}
          />
          <SummaryCard
            item={{
              label: 'Holiday',
              value: reportData.filter((d) => d.status === 'Holiday').length,
              indicator: 'blue',
            }}
          />
        </Box>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>
                            {fDate(row.attendance_date, 'DD/MM/YYYY')}
                          </TableCell>
                          <TableCell>
                      ...
                      <TableCell colSpan={8} align="center" sx={{ py: 10 }}>
                        {!fromDate || !toDate ? (
                          <Stack spacing={1} alignItems="center">
                            <Iconify
                              icon={'solar:filter-bold-duotone' as any}
                              width={48}
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1}>
        ...
        <Box sx={{ flexGrow: 1, pl: 1 }}>
          <Typography
            variant="subtitle2"
            sx={{ color: 'text.secondary', fontWeight: 700, mb: 0.2 }}
          >
            {item.label}
          </Typography>
