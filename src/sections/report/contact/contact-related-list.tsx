      ...
      return (
        <TableRow key={row.name} hover>
          ...
          <TableCell align="right" sx={{ fontWeight: 600 }}>
            {fCurrency(row.grand_total)}
          </TableCell>
          <TableCell align="right" sx={{ color: 'success.main', fontWeight: 600 }}>
            {fCurrency(row.received_amount)}
          </TableCell>
          <TableCell align="right" sx={{ color: 'error.main', fontWeight: 700 }}>
            {fCurrency(row.balance_amount)}
          </TableCell>
    ...
    if (type === 'deals') {
      return (
        <TableRow key={row.name} hover>
          ...
          <TableCell align="right">
            <IconButton color="primary" onClick={() => router.push(`/deals`)} size="small">
              <Iconify icon="solar:eye-bold" />
            </IconButton>
          </TableCell>
        </TableRow>
                    ...
                    {data.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={10}>
                          <EmptyContent title={`No ${type} available`} sx={{ py: 6 }} />
                        </TableCell>
                      </TableRow>
                    )}
                  </>
  ...
  return (
    ...
    >
      ...
      <Box>
        <Typography
          variant="caption"
          sx={{ color: 'text.secondary', fontWeight: 800, textTransform: 'uppercase' }}
        >
          {title}
        </Typography>
