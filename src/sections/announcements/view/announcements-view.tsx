...
export function AnnouncementsView() {
  ...
  const { data, total, refetch } = useAnnouncements(
    page + 1,
    rowsPerPage,
    filterName,
    filters,
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} announcement(s) deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      refetch();
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to delete announcements',
        severity: 'error',
      });
    }
  };
