...
export function InterviewDetailsDialog({ open, onClose, interview }: Props) {
  ...
  const renderApplicantDetails = (
    <Stack spacing={3}>
      ...
      <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
        ...
        {readField('Cover Letter', interview.cover_letter, {
          colSpan: true,
          multiline: true,
          rows: 3,
        })}
  ...
  const renderInterviewDetails = (
    <Stack spacing={3}>
      ...
      <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
        {readField(
          'Scheduled On',
          interview.scheduled_on ? new Date(interview.scheduled_on).toLocaleDateString('en-GB') : ''
        )}
        <Box />
  ...
  const renderPerformance = (
    <Stack spacing={3}>
      {readField('Overall Performance', interview.overall_performance, {
        multiline: true,
        rows: 3,
      })}
