...
export function EmployeeMonthlyAwardTableRow({ row, index, onView, onEdit, onDelete }: Props) {
  ...
  return (
    ...
    >
      ...
      <TableCell>{month ? fDate(month, 'MMMM YYYY').toUpperCase() : '-'}</TableCell>

      <TableCell align="center">
        <Typography variant="subtitle2">{Number(total_score || 0).toFixed(2)}</Typography>
      </TableCell>
      ...
      <TableCell align="center">
        <Label color={published ? 'success' : 'error'}>{published ? 'Published' : 'Draft'}</Label>
      </TableCell>
