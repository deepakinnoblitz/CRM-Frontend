...
export default function SalarySlipAutoAllocateDialog({ open, onClose, onSuccess, onError }: Props) {
  ...
  const handleGenerate = async () => {
    try {
      ...
      onSuccess(
        message || `Successfully generated salary slips for ${selectedEmployees.length} employee(s)`
      );
      onClose();
    } catch (error: any) {
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Box sx={{ mt: 2 }}>
              ...
              <Typography variant="body2" sx={{ mt: 2, color: 'text.secondary' }}>
                Select year and month, then choose employees to generate salary slips for{' '}
                <strong>
