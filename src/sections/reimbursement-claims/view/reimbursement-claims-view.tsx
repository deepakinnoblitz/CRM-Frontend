...
export function ReimbursementClaimsView() {
  ...
  const isHR = user?.roles.some((role: string) =>
    ['HR Manager', 'HR User', 'HR', 'System Manager', 'Administrator', 'Accounts Manager'].includes(
      role
    )
  );
  ...
  const handleApplyAction = async (id: string, action: string) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || `Failed to ${action} claim`,
        severity: 'error',
      });
    }
  };
  ...
  const handleCreate = async () => {
    ...
    const claimData = {
      ...
      ...(isEdit &&
      user?.roles.some((r) =>
        ['System Manager', 'HR', 'HR User', 'HR Manager', 'Accounts Manager'].includes(r)
      )
        ? {
            payment_reference: paymentReference,
            paid_date: paidDate || undefined,
          ...
          <DatePicker
            ...
            onChange={(newValue) => {
              ...
              if (fieldname === 'date_of_expense') {
                ...
                if (formErrors.dateOfExpense)
                  setFormErrors((prev) => ({ ...prev, dateOfExpense: '' }));
              } else {
            ...
            slotProps={{
              textField: {
                ...
                helperText:
                  formErrors[fieldname === 'date_of_expense' ? 'dateOfExpense' : fieldname],
                InputLabelProps: { shrink: true },
                sx: {
                  '& .MuiFormLabel-asterisk': {
                    color: 'red',
        ...
        <DialogContent dividers>
          <Box sx={{ display: 'grid', gap: 3, p: 2 }}>
            ...
            {renderField(
              ...
              'Claim Type',
              'select',
              claimTypes.map((type) => ({ value: type.name, label: type.name })),
            ...
            {renderField(
              ...
              'date',
              [],
              { inputProps: { readOnly: isApprovedOrPaid }, disabled: isApprovedOrPaid },
              true
            )}
            {renderField(
              'amount',
            ...
            {renderField('claim_details', 'Claim Details', 'textarea', [], {
            ...
            {isEdit &&
              ...
              (currentClaim?.workflow_state === 'Approved' ||
                currentClaim?.workflow_state === 'Paid') && (
                <>
                  <Typography
                    variant="subtitle2"
                    sx={{ color: 'text.secondary', mt: 1, gridColumn: { md: 'span 2' } }}
                  >
                    Settlement Details
                  </Typography>
                  ...
                  <FormControlLabel
                    control={
                      <Android12Switch checked={paid} onChange={(e) => setPaid(e.target.checked)} />
                    }
                    label="Paid"
                  ...
                  {paid && (
                    <>
                      ...
                      {renderField('payment_reference', 'Payment Reference', 'text', [], {
                        placeholder: 'Enter payment reference',
                      })}

                      {renderField('approver_comments', 'Notes', 'textarea', [], {
