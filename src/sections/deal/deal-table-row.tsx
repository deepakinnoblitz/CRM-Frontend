  ...
  return (
    ...
    >
      ...
      <TableCell component="th" scope="row">
        <Box sx={{ gap: 2, display: 'flex', alignItems: 'center' }}>{row.title}</Box>
      </TableCell>

      <TableCell>{row.account}</TableCell>

      <TableCell>{row.contactName ? `${row.contactName} (${row.contact})` : row.contact}</TableCell>

      <TableCell>
        {row.value
          ? new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(row.value)
          : '-'}
      </TableCell>
      ...
      <TableCell align="right">
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}>
          <IconButton
            onClick={() =>
              (window.location.href = `/estimations/new?deal_id=${encodeURIComponent(row.id)}&client_id=${encodeURIComponent(row.contact)}`)
            }
          ...
          <IconButton
            onClick={() =>
              (window.location.href = `/invoices/new?deal_id=${encodeURIComponent(row.id)}`)
            }
