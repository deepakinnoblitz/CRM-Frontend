...
import {
  type WorkflowAction,
  getLeaveAllocationWorkflowActions,
  applyLeaveAllocationWorkflowAction,
...
export function LeaveAllocationDetailsDialog({
  open,
  onClose,
  allocationId,
  onRefresh,
  ...
  const renderContent = allocation && (
    <Stack spacing={3.5} sx={{ marginTop: 3 }}>
      ...
      <Box>
        <Typography
          variant="overline"
          sx={{ color: 'text.disabled', fontWeight: 800, mb: 2, display: 'block' }}
        >
          Allocation Details
        </Typography>
        <Box
      ...
      <Box sx={{ p: 2.5, bgcolor: 'background.neutral', borderRadius: 2 }}>
        ...
        <LinearProgress
          variant="determinate"
          value={Math.min(
            (allocation.total_leaves_taken / allocation.total_leaves_allocated) * 100,
            100
          )}
          sx={{
            height: 8,
      ...
      {actions.length > 0 && isHR && (
        <Stack direction="row" spacing={1.5} justifyContent="flex-end" sx={{ pt: 1 }}>
          {actions.map((action) => {
            const isReject =
              action.action.toLowerCase().includes('reject') ||
              action.action.toLowerCase().includes('cancel');
            return (
              <LoadingButton
                key={action.action}
                variant="contained"
                ...
                sx={{
                  ...
                  border: (theme) =>
                    `1px solid ${alpha(isReject ? theme.palette.error.main : '#086ad8', 0.12)}`,
                  '&:hover': {
                    bgcolor: (theme) =>
                      alpha(isReject ? theme.palette.error.main : '#086ad8', 0.16),
                  },
                }}
              >
                {action.action}
        ...
        <DialogContent sx={{ p: 3 }}>
          {loading && !allocation ? (
            <Stack alignItems="center" justifyContent="center" sx={{ py: 10, minHeight: 200 }}>
              ...
              >
                <Iconify
                  icon={'svg-spinners:ring-resize' as any}
                  width={40}
