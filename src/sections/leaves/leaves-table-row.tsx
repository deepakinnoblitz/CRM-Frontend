...
export function LeavesTableRow({
  ...
  const getActionIcon = (action: string): any => {
    ...
    if (lower.includes('clarification') || lower.includes('query'))
      return 'solar:question-square-bold';
  ...
  const showActions =
    isHR &&
    (row.status === 'Pending' || row.status === 'Open' || row.status === 'Clarification Requested');
        ...
        <TableCell align="center" sx={{ display: { xs: 'none', md: 'table-cell' } }}>
          {!isPermission && (
            <Typography variant="body2" noWrap>
              ...
              {row.halfDay === 1 && (
                <Iconify
                  icon={'solar:history-bold' as any}
                  width={16}
                  sx={{ ml: 0.5, color: 'info.main', verticalAlign: 'middle' }}
                />
              )}
            </Typography>
          )}
        ...
        {loadingActions ? (
          ...
          <>
            ...
            {actions.length === 0 && (
              <Typography
                variant="caption"
                sx={{ p: 1, color: 'text.disabled', textAlign: 'center', display: 'block' }}
              >
