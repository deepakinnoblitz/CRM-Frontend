...
import {
  ...
  applyLeaveWorkflowAction,
  getEmployeeProbationInfo,
} from 'src/api/leaves';
...
export function LeavesView() {
  ...
  useEffect(() => {
    if (employee) {
      getEmployeeProbationInfo(employee).then(setProbationInfo).catch(console.error);
    } else {
  ...
  useEffect(() => {
    ...
    if (employee && leaveType && fromDate && toDate) {
      checkLeaveBalance({
        ...
        permission_hours:
          leaveType.toLowerCase() === 'permission' ? Number(permissionHours) : undefined,
      })
        .then((res) => {
  ...
  const validateForm = () => {
    ...
    if (
      leaveType.toLowerCase() === 'permission' &&
      (!permissionHours || Number(permissionHours) < 10)
    ) {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (
        ...
        setSnackbar({
          open: true,
          message: 'Permission duration must be at least 10 minutes',
          severity: 'error',
        });
        return;
      }
      ...
      setSnackbar({
        open: true,
        message: err.message || 'Error saving leave application',
        severity: 'error',
      });
    } finally {
      setCreating(false);
  ...
  const handleApplyAction = async (id: string, action: string) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || `Failed to ${action} leave application`,
        severity: 'error',
      });
    }
  };
      ...
      <Dialog open={openCreate} onClose={handleCloseCreate} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
              ...
              {balanceInfo && (
                <Box
                  sx={{
                    ...
                    borderRadius: 2,
                    bgcolor: (theme) =>
                      balanceInfo.remaining >= totalDays
                ...
                >
                  ...
                  >
                    ...
                    {leaveType.toLowerCase() === 'permission' ? 'Minutes' : 'Days'}
                  </Typography>
                </Box>
              )}
            </Box>
          ...
          {leaveType && balanceInfo && totalDays > balanceInfo.remaining && (
            <Alert severity="error" sx={{ mb: 3, borderRadius: 2 }}>
              Insufficient {leaveType} balance. You have{' '}
              <strong>
                {balanceInfo.remaining}{' '}
                {leaveType.toLowerCase() === 'permission' ? 'minutes' : 'days'}
              </strong>{' '}
              available, but you are requesting{' '}
              <strong>
                {totalDays} {leaveType.toLowerCase() === 'permission' ? 'minutes' : 'days'}
              </strong>
              .
            </Alert>
          )}

          <Box display="grid" margin={2} gridTemplateColumns="1fr" gap={3}>
            ...
            >
              ...
              {leaveTypeOptions.map((option) => (
                <option
                  ...
                  value={option.name}
                  disabled={
                    probationInfo?.is_probation &&
                    probationInfo.restricted_types.includes(option.name)
                  }
                >
                  {option.name}{' '}
                  {probationInfo?.is_probation &&
            ...
            {employee && leaveType && (
              <>
                ...
                {leaveType &&
                  (leaveType.toLowerCase() === 'paid leave' ||
                    leaveType.toLowerCase() === 'unpaid leave') && (
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={halfDay}
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  ...
                  {leaveType.toLowerCase() === 'permission' || halfDay ? (
                    <DatePicker
                      label={
                        leaveType.toLowerCase() === 'permission' ? 'Permission Date' : 'Leave Date'
                      }
                  ...
                  {leaveType && leaveType.toLowerCase() === 'permission' && (
                    <TimePicker
                      ...
                      value={
                        permissionHours
                          ? dayjs().startOf('day').add(Number(permissionHours), 'minutes')
                          : null
                      }
                      ...
                      slotProps={{
                        textField: {
                          ...
                          required: true,
                          helperText:
                            formErrors.permissionHours ||
                            'Select hours and minutes. Minimum 10 minutes required.',
        ...
        <DialogActions>
          <Button
            ...
            disabled={
              creating || uploading || (balanceInfo && totalDays > balanceInfo.remaining) || false
            }
