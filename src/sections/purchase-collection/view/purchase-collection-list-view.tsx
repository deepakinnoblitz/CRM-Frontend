...
export function PurchaseCollectionListView({ hideHeader }: Props) {
    ...
    const fetchOptions = async () => {
      try {
        const [vendorsRes, paymentTypesRes] = await Promise.all([
          fetch(
            '/api/method/frappe.client.get_list?doctype=Contacts&fields=["name","first_name"]&link_full_match=0&limit_page_length=999&filters=[["Contacts","customer_type","=","Purchase"]]',
            {
              credentials: 'include',
            }
          ),
  ...
  const handleFilterName = useCallback((value: string) => {
    setFilterName(value);
    setPage(0);
  }, []);
      ...
      <Card>
        <PurchaseCollectionTableToolbar
          ...
          onSortChange={(value: string) => {
            const [field, direction] = value.split('_').reduce(
