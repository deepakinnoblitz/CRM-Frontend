...
export function PurchaseForm({ id }: Props) {
  ...
  const [vendorOptions, setVendorOptions] = useState<
    { name: string; first_name: string; company_name: string }[]
  >([]);
  ...
  const handleRemoveItem = (index: number) => {
    ...
    const { totalQty, totalAmount, grandTotal } = calculateTotals(
      newItems,
      formData.overall_discount,
      formData.overall_discount_type
    );
    setFormData({
      ...formData,
      items: newItems,
  ...
  const handleOverallDiscountChange = (field: string, value: any) => {
    const { totalQty, totalAmount, grandTotal } = calculateTotals(
      formData.items,
      field === 'overall_discount' ? value : formData.overall_discount,
      field === 'overall_discount_type' ? value : formData.overall_discount_type
    );
    setFormData({
      ...formData,
      [field]: value,
    ...
    <DashboardContent maxWidth={false}>
      ...
      <Card sx={{ p: 3 }}>
        ...
        >
          ...
          <Autocomplete
            ...
            value={
              ...
              (formData.vendor_name
                ? { name: formData.vendor_name, first_name: formData.vendor_name, company_name: '' }
                : null)
            }
          ...
          >
            {['Next day Payment', 'Due On Receipt', '15 days', '30 days', '60 days', '1 Year'].map(
              (opt) => (
                        ...
                        <TableCell sx={{ minWidth: 250 }}>
                          <Autocomplete
                            ...
                            getOptionLabel={(option: any) => {
                              if (typeof option === 'string') return option;
                              return (
                                option.display_name || option.item_name || option.item_code || ''
                              );
                            }}
                            value={
                              (itemOptions as any[]).find(
                                (opt) => opt.item_code === item.service
                              ) ||
                              (item.service
                                ? {
          ...
          >
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr', gap: 2 }}>
              <TextField fullWidth label="Total Amount" value={formData.total_amount} disabled />
              <TextField fullWidth label="Total Quantity" value={formData.total_qty} disabled />
              <Box sx={{ display: 'flex', gap: 1 }}>
                <TextField
                  select
                  fullWidth
                  ...
                  onChange={(e) =>
                    handleOverallDiscountChange('overall_discount_type', e.target.value)
                  }
                >
                  <MenuItem value="Flat">Flat</MenuItem>
              ...
              <TextField fullWidth label="Grand Total" value={formData.grand_total} disabled />
              <TextField
                fullWidth
                label="Paid Amount"
                type="number"
