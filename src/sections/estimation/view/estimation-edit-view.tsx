...
import {
  getEstimation,
  updateEstimation,
  getEstimationPrintUrl,
  convertEstimationToInvoice,
} from 'src/api/estimation';
...
export function EstimationEditView() {
          ...
          if (data.attachments) {
            try {
              ...
              if (Array.isArray(parsed)) {
                ...
                setAttachments([
                  {
                    name: data.attachments.split('/').pop() || 'Attachment',
              ...
              setAttachments([
                { name: data.attachments.split('/').pop() || 'Attachment', url: data.attachments },
              ]);
            }
          } else {
            setAttachments([]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: err.message || 'Failed to update estimation',
        severity: 'error',
      });
    } finally {
      setLoading(false);
        ...
        <Card sx={{ p: 4 }}>
          ...
          >
            ...
            <Autocomplete
              ...
              options={dealOptions}
              getOptionLabel={(option) =>
                option.deal_title ? `${option.name} - ${option.deal_title}` : option.name || ''
              }
              value={dealOptions.find((opt) => opt.name === deal) || null}
              onChange={(_e, newValue) => setDeal(newValue?.name || '')}
              renderInput={(params) => <TextField {...params} label="Link Deal" />}
            />
