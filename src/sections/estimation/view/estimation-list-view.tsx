  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        setCustomerOptions(
          contacts.map((c: any) => ({ name: c.name, customer_name: c.first_name || c.name }))
        );
        ...
        const refs = Array.from(
          new Set(estimations.map((e: any) => e.ref_no).filter(Boolean))
        ) as string[];
  ...
  const renderContent = (
    <>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
        {!hideTitle && <Typography variant="h4">Estimations</Typography>}
