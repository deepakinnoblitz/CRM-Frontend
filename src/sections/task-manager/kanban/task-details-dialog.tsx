...
function StatusBadge({
  ...
  conf: { color: string; bg: string; border: string; icon: string };
  label: string;
}) {
  return (
    <Box
    ...
    >
      ...
      <Typography variant="caption" sx={{ fontWeight: 800, color: conf.color, lineHeight: 1 }}>
        {label}
      </Typography>
    </Box>
  );
  ...
  return (
    <Stack direction="row" alignItems="center" spacing={1.5} sx={{ minWidth: 0 }}>
      ...
      <Box sx={{ minWidth: 0 }}>
        ...
        <Typography variant="body2" sx={{ fontWeight: 700, mt: 0.1, wordWrap: 'break-word' }}>
          {value}
        </Typography>
      </Box>
    </Stack>
...
export default function TaskDetailsDialog({
  ...
  useEffect(() => {
    if (open && initialTask?.name) {
      ...
      getTaskManager(initialTask.name)
        .then((fullTask) => {
          setTask((prev) => ({
            ...prev,
            ...fullTask,
            assignees: prev?.assignees || fullTask.assignees,
          }));
        })
        .catch((err) => {
  ...
  const handleConfirmClose = async (hours: string, remarks: string, attachment?: string) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error?.message || 'Failed to close task',
        severity: 'error',
      });
    } finally {
      setLoading(false);
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            >
              ...
              >
                ...
                >
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    >
                      <Iconify
                        icon={'solar:clipboard-list-bold' as any}
                        width={24}
                        sx={{ color: '#14b8a6' }}
                      />
                    </Box>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1 }}>
                <InfoRow
                  icon="solar:folder-favourite-bookmark-bold"
                  iconColor="#6366f1"
                  label="Project"
                  value={task.project || 'N/A'}
                />
                <InfoRow
                  icon="solar:buildings-bold"
                  iconColor="#8b5cf6"
                  label="Department"
                  value={task.department || 'N/A'}
                />
                <InfoRow
              ...
              <Stack direction="row" spacing={1.5} flexWrap="wrap" useFlexGap>
                ...
                {(task.status === 'Open' || task.status === 'Reopened') && (
                  <Button
                    ...
                    onClick={() =>
                      handleAction(() => acceptTaskManager(task.name), 'Task Accepted')
                    }
                ...
                {(() => {
                  const isTaskManager =
                    user?.roles?.includes('Task Manager') ||
                  ...
                  >
                    <Avatar
                      ...
                      sx={{
                        ...
                        fontSize: 13,
                        fontWeight: 800,
                        bgcolor: avatarColor(assignee.employee_name),
                        flexShrink: 0,
                      }}
                    >
