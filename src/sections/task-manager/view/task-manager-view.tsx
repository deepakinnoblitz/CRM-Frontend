...
export default function TaskManagerView() {
  ...
  const kanbanTasks = sortTasks(
    baseTasks.filter((task) => {
      ...
      if (kanbanFilters.priority !== 'all' && task.priority !== kanbanFilters.priority)
        return false;
      if (kanbanFilters.due_date && task.due_date !== kanbanFilters.due_date) return false;
      if (kanbanFilters.department !== 'all' && task.department !== kanbanFilters.department)
        return false;
  ...
  return (
    <DashboardContent maxWidth={false}>
      <Container
        maxWidth="xl"
        sx={{ height: 1, display: 'flex', flexDirection: 'column', px: { xs: 2, md: 1 } }}
      >
