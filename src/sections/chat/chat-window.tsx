...
export default function ChatWindow({
  ...
  const lastMessageTime = lastMessage
    ? fDateSeparator(lastMessage.send_date) + ' ' + fDateTime(lastMessage.send_date, 'h:mm A')
    : '';
              ...
              prev.map((msg) =>
                msg.message_name === data.message_name
                  ? {
                      ...msg,
                      content:
                        '<p class="deleted-message-text" style="color:#666; font-style:italic; margin:0;">This message was deleted</p>',
                      is_media: 0,
                      is_voice_clip: 0,
                      is_document: 0,
                      attachment: '',
  ...
  const handleSend = useCallback(
    async (data: {
      ...
      const localMsg = {
        ...
        content:
          content ||
          (data.is_voice_clip ? '🎤 Voice Message' : data.is_media ? '📷 Image' : '📄 File'),
      ...
      try {
        ...
        if (newMessageName) {
          setMessages((prev) =>
            prev.map((m) =>
              m.id_message_local_from_app === localId ? { ...m, message_name: newMessageName } : m
            )
          );
        }
    ...
    <Stack sx={{ height: 1 }}>
        ...
        >
          <Avatar
            ...
            sx={{
              ...
              color:
                channel.avatar_url || channel.channel_image || channel.channel_info?.avatar
                  ? 'text.secondary'
      ...
      >
        {messages.map((msg, index) => {
          ...
          const isSameDate =
            prevMsg && fDateSeparator(prevMsg.send_date) === fDateSeparator(msg.send_date);
          ...
          const isConcluded =
            ...
            messages.some((m, idx) => {
              ...
              const isEndLog =
                /CALL_(ENDED|MISSED)/i.test(mType) || /CALL_(ENDED|MISSED)/i.test(mText);
      ...
      {/* Input */}
      {channel.chat_status !== 'Closed' && <ChatInput onSend={handleSend} />}
...
const ChatMessageItem = memo(
  ({
    ...
    const callLogMetadata =
      (msg.message_type?.includes('[CALL_') ? msg.message_type : msg.content) || '';
      ...
      return (
        ...
        >
          ...
          <Typography
            variant="caption"
            sx={{
              color: isCallMissed ? 'error.main' : 'text.disabled',
              fontWeight: isCallMissed ? 'bold' : 'normal',
            }}
          >
            {isCallMissed
              ? isGroup
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        {!isSameDate && (
          <Stack direction="row" alignItems="center" spacing={2} sx={{ my: 1 }}>
            ...
            <Typography
              variant="caption"
              sx={{ color: 'text.disabled', fontWeight: 'fontWeightBold' }}
            >
            ...
            >
              ...
              <Box
                sx={{
                  ...
                  border: (theme) =>
                    isCallInvite && isConcluded
                      ? `1px solid ${alpha(theme.palette.info.main, 0.15)}`
                      : 'none',
                  boxShadow: (theme) => {
                    if (isCallInvite && !isConcluded)
                      return `0 12px 24px ${alpha(theme.palette.primary.main, 0.2)}`;
                    if (isCallInvite && isConcluded)
                      return `0 4px 12px ${alpha(theme.palette.info.main, 0.1)}`;
                    return 'none';
                  },
                  ...
                  <Typography
                    ...
                    sx={{
                      color: isConcluded
                        ...
                        : isMe
                          ? '#fff'
                          : stringToDarkColor(msg.sender || msg.sender_email || ''),
                      fontWeight: 'bold',
                      display: 'block',
                      mb: 1.5,
                      fontSize: '11px',
              ...
              <InputAdornment position="end">
                {message.trim() ? (
                  <IconButton onClick={handleSend} edge="end">
                    <Box
                      component="img"
                      src={`${CONFIG.assetsDir}/icons/send_icon.png`}
