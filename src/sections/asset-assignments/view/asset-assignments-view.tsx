...
export function AssetAssignmentsView() {
  ...
  const { data, total, refetch } = useAssetAssignments(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleSortChange = (value: string) => {
    if (value === 'date_desc') {
      ...
      setOrderBy('employee_name');
      setOrder('desc');
    } else if (value === 'asset_asc') {
      setOrderBy('asset_name');
      setOrder('asc');
    } else if (value === 'asset_desc') {
      setOrderBy('asset_name');
      setOrder('desc');
    }
    setPage(0);
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} assignment(s) deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      refetch();
  ...
  const handleEditRow = useCallback(
    (row: any) => {
      ...
      setSelectedAsset(
        assets.find((a) => a.name === row.asset) || { name: row.asset, asset_name: row.asset_name }
      );
                  ...
                  <TableRow>
                    <TableCell colSpan={isHR ? 6 : 5}>
                      <EmptyContent
                        ...
                        description={
                          isHR
                            ? "You haven't created any asset assignments yet. Click 'New Assignment' to get started."
                            : "You haven't been assigned any assets yet."
                        }
                        icon={
                          isHR ? 'solar:clipboard-list-bold-duotone' : 'solar:laptop-bold-duotone'
                        }
                      />
                    </TableCell>
                  </TableRow>
