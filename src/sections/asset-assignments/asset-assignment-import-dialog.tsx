...
export function AssetAssignmentImportDialog({ open, onClose, onRefresh }: Props) {
  ...
  useEffect(() => {
    if (open) {
      getDocFields('Asset Assignment').then(setDocFields).catch(console.error);
    }
  }, [open]);
    ...
    try {
      ...
      preview.columns.forEach((col: any, index: number) => {
        ...
        const field = docFields.find((f) => {
          ...
          const fieldname = f.fieldname?.trim().toLowerCase();
          const fieldnameSpaced = f.fieldname?.replace(/_/g, ' ').toLowerCase();

          return label === header || fieldname === header || fieldnameSpaced === header;
        });
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      const fullGrid = [
        headers,
        ...editedData.map((row: any) => dataIndices.map((idx: number) => row[idx])),
      ];
      await updateImportFile(importName, fullGrid);

      // 2. Prepare final mapping for Frappe: indices must match the CSV we just uploaded
      // Since fullGrid starts at 0 for the first data column, we map accordingly
      const finalMapping: Record<string, string> = {};
        ...
        if (['Success', 'Partial Success', 'Error', 'Timed Out'].includes(status.status)) {
          ...
          if (warnings && warnings.length > 0) {
            ...
            const warningMsgs = warnings
              .map((w: any) => (w.row ? `Row ${w.row}: ${w.message}` : w.message))
              .join('\n');
  ...
  const renderUpload = (
    <Box sx={{ py: 4, display: 'flex', flexFlow: 'column', alignItems: 'center' }}>
      ...
      <Button
        ...
        onClick={() => window.open(getTemplateUrl('Asset Assignment'), '_blank')}
        sx={{ mt: 2 }}
      >
        ...
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>Asset Assignment Field</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {previewData?.columns
                  ...
                  <TableRow key={actualIdx}>
                    <TableCell>
                      <Typography variant="subtitle2">
                        {col.header_title || `Column ${actualIdx + 1}`}
                      </Typography>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <FormControl fullWidth size="small">
                          <Select
                            ...
                            onChange={(e) =>
                              setMapping({ ...mapping, [actualIdx]: e.target.value })
                            }
                          >
    ...
    <Box sx={{ mt: 2 }}>
      {(() => {
        const filteredWarnings = (previewData?.warnings || []).filter((w: any) => {
          ...
          if (
            w.type === 'info' &&
            (message.includes('mapping column') ||
              message.includes('cannot match column') ||
              message.includes('with any field'))
          )
            return false;
          return true;
        });
          ...
          <Stack spacing={1} sx={{ mb: 3 }}>
            ...
            {filteredWarnings.length > 5 && (
              <Typography variant="caption">
                ...and {filteredWarnings.length - 5} more warnings
              </Typography>
            )}
          </Stack>
        );
                  ...
                  >
                    ...
                    <Box sx={{ px: 0.5 }}>
                      ...
                      >
                        {isSkipped
                          ? "Don't Import"
                          : docFields.find((f) => f.fieldname === mapping[i])?.label || mapping[i]}
                      </Typography>
                    </Box>
                  </TableCell>
                );
                    ...
                    <TableCell
                      ...
                      sx={{
                        ...
                        bgcolor: (() => {
                          ...
                          const isMandatory = docFields.find(
  ...
  const renderImport = (
    <Box sx={{ py: 4 }}>
      <Stack spacing={3}>
        ...
        {importStatus && (
          <Box
            sx={{ p: 2, bgcolor: 'background.neutral', borderRadius: 1, display: 'flex', gap: 4 }}
          >
            <Box>
                            ...
                            >
                              {log.messages !== '[]'
                                ? JSON.parse(log.messages)
                                    .map((m: any) => getFriendlyErrorMessage(m))
                                    .join('\n')
