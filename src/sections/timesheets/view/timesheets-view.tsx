...
export function TimesheetsView() {
  ...
  const { data, total, refetch } = useTimesheets(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const canReset = filters.employee !== 'all' || !!filters.startDate || !!filters.endDate;
          ...
          <Box sx={{ display: 'grid', gap: 3, p: 2 }}>
            ...
            >
              <Autocomplete
                ...
                value={
                  employees.find((emp) => emp.name === employee) ||
                  (employee && user?.employee === employee
                    ? { name: user.employee, employee_name: user.employee_name }
                    : null)
                }
                readOnly={isEdit}
