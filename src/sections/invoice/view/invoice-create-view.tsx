...
export function InvoiceCreateView() {
  ...
  const [customerId, setCustomerId] = useState(
    estimationData?.client_name || estimationData?.customer_id || ''
  );
  ...
  const [attachments, setAttachments] = useState<any[]>(() => {
    try {
      return estimationData?.attachments
        ? typeof estimationData.attachments === 'string'
          ? JSON.parse(estimationData.attachments)
          : estimationData.attachments
        : [];
    } catch {
      return [];
    }
  ...
  const [discountType, setDiscountType] = useState<'Flat' | 'Percentage'>(
    estimationData?.overall_discount_type || 'Flat'
  );
  ...
  useEffect(() => {
    getDoctypeList('Contacts', ['name', 'first_name', 'company_name', 'address']).then(
      setCustomerOptions
    );
    getDoctypeList('Item', ['name', 'item_name', 'rate', 'item_code']).then(setItemOptions);
    getDoctypeList('Tax Types', ['name', 'tax_name', 'tax_percentage', 'tax_type']).then(
      setTaxOptions
    );
  ...
  const handleItemChange = async (index: number, field: keyof ItemRow, value: any) => {
    ...
    const discountAmount =
      item.discount_type === 'Flat' ? item.discount : (amount * item.discount) / 100;
  ...
  return (
    <DashboardContent maxWidth="xl">
        ...
        <Card sx={{ p: 4 }}>
          ...
          >
            <Autocomplete
              ...
              value={
                dealOptions.find((opt) => opt.name === dealId) || (dealId ? { name: dealId } : null)
              }
              onChange={(_e, newValue) => setDealId(newValue?.name || '')}
              renderInput={(params) => <TextField {...params} label="Linked Deal" />}
            ...
            <Stack direction="row" spacing={1} alignItems="center">
              <Autocomplete
                ...
                getOptionLabel={(option) =>
                  option.first_name ? `${option.name} - ${option.first_name}` : option.name || ''
                }
                value={customerOptions.find((opt) => opt.name === customerId) || null}
                onChange={(_e, newValue) => handleCustomerChange(newValue?.name || '')}
                renderInput={(params) => <TextField {...params} label="Customer ID" required />}
              />
              {customerId && (
                <Button
                  variant="contained"
                        ...
                        component: (
                          <Stack direction="row" spacing={0.5} alignItems="center">
                            ...
                            <TextField
                              ...
                              onFocus={(e) => e.target.select()}
                              inputProps={{
                                sx: { textAlign: 'right', typography: 'body2', px: 0 },
                              }}
                              InputProps={{ disableUnderline: true }}
                            />
                          </Stack>
            ...
            <Stack spacing={2} sx={{ width: 400, mt: 3 }}>
              <Stack direction="row" alignItems="center" justifyContent="space-between">
                <Stack direction="row" alignItems="center" spacing={1}>
                  <Iconify
                    icon={'solar:box-bold-duotone' as any}
              ...
              <Stack direction="row" alignItems="center" justifyContent="space-between">
                <Stack direction="row" alignItems="center" spacing={1}>
                  <Iconify
                    icon={'solar:calculator-minimalistic-bold-duotone' as any}
                    sx={{ color: 'text.secondary' }}
                  />
                  <Typography variant="body2" color="text.secondary">
                    Total Tax
              ...
              <Stack direction="row" alignItems="center" justifyContent="space-between">
                <Stack direction="row" alignItems="center" spacing={1.5}>
                  <Iconify
                    icon={'solar:wad-of-money-bold-duotone' as any}
                ...
                <Typography variant="h6" color="primary" sx={{ width: 120, textAlign: 'right' }}>
      ...
      <Dialog
        open={itemDialogOpen}
        onClose={() => !creatingItem && setItemDialogOpen(false)}
        fullWidth
        maxWidth="xs"
      >
