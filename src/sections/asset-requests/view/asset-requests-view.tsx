...
import {
  ...
  fetchPendingAssetRequests,
} from 'src/api/asset-requests';
...
export function AssetRequestsView() {
  ...
  const fetchMyReqs = useCallback(async () => {
    ...
    try {
      ...
      if (isHR) {
        const result = await fetchPendingAssetRequests(
          myPage + 1,
          myRowsPerPage,
          myFilters.type,
          myFilters.status,
  ...
  const handleSubmitRequest = async () => {
    ...
    if ((isNewOrDec && !assetCategory) || (isReturn && !returnAsset) || !purpose) {
      setSnackbar({
        open: true,
        message: 'Please fill in all mandatory fields (including Asset to Return).',
        severity: 'error',
      });
      return;
    }
    ...
    try {
      ...
      if (returnPendingFile) {
        ...
        const uploadRes = await frappeRequest('/api/method/upload_file', {
          method: 'POST',
          body: formDataUpload,
        });
  ...
  const handleHrAction = async () => {
    ...
    try {
      if (hrAction === 'approve' && hrActionRequest.request_type === 'Declaration') {
        ...
        if (
          hrAction === 'approve' &&
          hrActionRequest.request_type === 'Return Request' &&
          hrReturnDate
        ) {
  ...
  const filteredMyReqs = myRequests.filter(
    (r) =>
      !mySearch ||
  ...
  const filteredHrReqs = hrRequests.filter(
    (r) =>
      !hrSearch ||
      ...
      {activeTab === 'my-requests' && (
        <Card>
          <AssetReqToolbar
            ...
            searchPlaceholder={
              isHR ? 'Search all requests by employee, type...' : 'Search my requests...'
            }
            onOpenFilter={() => setMyFiltersOpen(true)}
            canReset={
              myFilters.type !== 'all' ||
              myFilters.category !== 'all' ||
              myFilters.status !== 'all' ||
              myFilters.priority !== 'all' ||
              !!myFilters.startDate ||
              !!myFilters.endDate
            }
            sortBy={mySort === 'modified desc' ? 'date_desc' : 'date_asc'}
            onSortChange={(val: string) => {
              setMySort(val === 'date_desc' ? 'modified desc' : 'modified asc');
              setMyPage(0);
            }}
            sortOptions={[
                  ...
                  {myLoading ? (
                    ...
                    <>
                                ...
                                <Stack spacing={1.5} sx={{ p: 1.5, minWidth: 240 }}>
                                  <Stack direction="row" alignItems="center" spacing={1}>
                                    <Iconify
                                      ...
                                      sx={{ color: '#08a3cd', width: 20, height: 20 }}
                                    />
                                    <Typography
                                      variant="subtitle2"
                                    ...
                                    <Box>
                                      ...
                                      <Typography
                                        ...
                                        sx={{
                                          ...
                                          p: 1,
                                          borderRadius: 1,
                                          border: '1px solid rgba(8, 163, 205, 0.16)',
                                        }}
                                      >
                                        {row.purpose}
                                      </Typography>
                              ...
                              slotProps={{
                                tooltip: {
                                  sx: {
                                    ...
                                    borderRadius: 2,
                                    boxShadow: (theme: any) =>
                                      theme.customShadows?.z24 || '0 24px 48px 0 rgba(0,0,0,0.16)',
                                ...
                                arrow: {
                                  sx: {
                                    ...
                                    '&::before': { border: '1px solid rgba(8, 163, 205, 0.4)' },
                                  },
                                },
                              }}
                              arrow
                      ...
                      <TableEmptyRows
                        height={68}
                        emptyRows={
                          filteredMyReqs.length > 0 && filteredMyReqs.length < 5
                            ? 5 - filteredMyReqs.length
                            : 0
                        }
                      />
      ...
      {activeTab === 'hr-dashboard' && isHR && (
        <Card>
          <AssetReqToolbar
            ...
            canReset={
              hrFilters.type !== 'all' ||
              hrFilters.category !== 'all' ||
              hrFilters.priority !== 'all' ||
              !!hrFilters.startDate ||
              !!hrFilters.endDate
            }
            sortBy={hrSort === 'modified desc' ? 'date_desc' : 'date_asc'}
            onSortChange={(val: string) => {
              setHrSort(val === 'date_desc' ? 'modified desc' : 'modified asc');
              setHrPage(0);
            }}
            sortOptions={[
              { value: 'date_desc', label: 'Newest First' },
                  ...
                  {hrLoading ? (
                    ...
                    <>
                          ...
                          <TableCell sx={{ minWidth: 180, maxWidth: 350 }}>
                              ...
                              title={
                                <Stack spacing={1.5} sx={{ p: 1.5, minWidth: 240 }}>
                                  ...
                                  <Box>
                                    ...
                                    >
                                      {row.asset_category
                                        ? `${row.asset_category}${row.asset_name || row.asset ? ` / ${row.asset_name || row.asset}` : ''}`
                                        : row.asset_name || row.asset || '-'}
                                    </Typography>
                                  </Box>
                            ...
                            >
                              <Box sx={{ cursor: 'pointer' }}>
                                ...
                                {row.purpose &&
                                  (row.asset_category || row.asset_name || row.asset) && (
                                    <Typography
                                      variant="caption"
                                      color="text.secondary"
                                      noWrap
                      ...
                      <TableEmptyRows
                        height={68}
                        emptyRows={
                          filteredHrReqs.length > 0 && filteredHrReqs.length < 5
                            ? 5 - filteredHrReqs.length
                            : 0
                        }
                      />
          ...
          <TablePagination
            ...
            rowsPerPageOptions={[10, 25, 50]}
            onRowsPerPageChange={(e) => {
              setHrRowsPerPage(parseInt(e.target.value, 10));
              setHrPage(0);
            }}
          />
        </Card>
          ...
          <Box sx={{ display: 'grid', gap: 3, mt: 1 }}>
            ...
            <FormControl fullWidth required disabled={isHR}>
              ...
              >
                ...
                <MenuItem value="Return Request">
                  Return Request — I want to return an asset
                </MenuItem>
              </Select>
            </FormControl>
                ...
                <Box sx={{ border: '1px dashed', borderColor: 'divider', borderRadius: 2, p: 2 }}>
                  ...
                  >
                    <Typography variant="caption" color="text.secondary">
                      Attach photo, receipt, or document as proof
                    </Typography>
                    <Button
                      component="label"
                      variant="outlined"
                  ...
                  {!returnAttachment && !returnPendingFile ? (
                    ...
                    >
                      ...
                      >
                        {returnPendingFile
                          ? returnPendingFile.name
                          : returnAttachment.split('/').pop() || returnAttachment}
                      </Typography>
                      <Button
                        size="small"
                        color="inherit"
            ...
            {hrAction === 'approve' && hrActionRequest?.request_type === 'Declaration' && (
              <Box sx={{ display: 'grid', gap: 3, pt: 1 }}>
                <Alert severity="info" sx={{ mb: 1 }}>
                  Fill out the remaining details to register{' '}
                  <strong>{hrActionRequest?.asset_name}</strong> and approve the request.
                </Alert>
                ...
                >
                  ...
                  <Autocomplete
                    ...
                    getOptionLabel={(o: any) => o.category_name || o.name || ''}
                    renderInput={(params) => (
                      <TextField {...params} label="Category" required sx={fieldSx} />
                    )}
                  />
                  <TextField
                    fullWidth
                    ...
                    onChange={(e) =>
                      setDecPurchaseCost(e.target.value === '' ? '' : Number(e.target.value))
                    }
                    sx={fieldSx}
                  />
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker
                      ...
                      onChange={(newValue) =>
                        setDecPurchaseDate(newValue?.format('YYYY-MM-DD') || '')
                      }
                      slotProps={{
                        textField: {
                          fullWidth: true,
            ...
            {hrAction === 'approve' && hrActionRequest?.request_type === 'Return Request' && (
              <Box sx={{ display: 'grid', gap: 2 }}>
                <Alert severity="warning">
                  Approving this Return will mark the asset as <strong>Available</strong> and close
                  the assignment.
                </Alert>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DatePicker
