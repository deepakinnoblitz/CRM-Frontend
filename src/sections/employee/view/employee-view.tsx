        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
          ...
          >
            ...
            <TextField
              ...
              inputProps={{
                sx: { textAlign: 'right', typography: 'body2', fontWeight: 600, p: 0, width: 110 },
              }}
...
export function EmployeeView() {
      ...
      .then((meta) => {
        ...
        meta.fields.forEach((f: any) => {
          ...
          if (field.depends_on) {
            ...
            try {
              field._visibility_fn = new Function(
                'formData',
        ...
        if (
          !meta.fields.find((f: any) => f.fieldname === 'designation' && f.fieldtype === 'Link')
        ) {
  ...
  const totals = useMemo(() => {
    ...
    const total_earnings = earnings.reduce(
  ...
  const handleSalaryRowChange = useCallback(
    (index: number, type: 'Earning' | 'Deduction', field: string, value: any) => {
  ...
  const handleDefaultSplitting = async () => {
    ...
    if (ctcValue <= 0) {
      setSnackbar({
        open: true,
        message: 'Please enter a valid CTC amount first',
        severity: 'warning',
      });
      return;
    }
    ...
    try {
      ...
      if (defaults.length === 0) {
        setSnackbar({
          open: true,
          message: 'No default salary components found. Please configure them in Masters.',
          severity: 'warning',
        });
        return;
      }
      ...
      setSnackbar({
        open: true,
        message: 'Default splitting applied successfully',
        severity: 'success',
      });
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} employees deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      await refetch();
      ...
      setSnackbar({
        open: true,
        message: e.message || 'Error during bulk delete',
        severity: 'error',
      });
    }
  };
  ...
  const validateForm = (): {
    ...
    const incompleteDeductions = (formData.deductions || []).some(
      (row: any) => !row.component_name
    );
  ...
  const handleCreate = async () => {
    ...
    if (!validationResult.isValid) {
      ...
      const errorMessage =
        validationResult.firstErrorMessage || 'Please correct the errors in the form';
  ...
  const handleEditRow = async (id: string) => {
    try {
      ...
      if (fullDoc) {
        ...
        if (cleanedRow.office_phone_number)
          cleanedRow.office_phone_number = cleanPhoneNumber(cleanedRow.office_phone_number);
      ...
      return (
        <Autocomplete
          ...
          onChange={(event, newValue: any) => {
            if (
              newValue?.isNew ||
              newValue === 'Create Department' ||
              newValue?.name === 'Create Department'
            ) {
              setOpenDepartmentCreate(true);
              setDepartmentSearch(newValue?.inputValue || '');
            } else {
              const value =
                typeof newValue === 'object' && newValue?.name ? newValue.name : newValue;
              handleInputChange(fieldname, value || '');
            }
          }}
          filterOptions={(currentOptions, params) => {
            ...
            const hasCreateOption = filtered.some(
              (option: any) =>
                (typeof option === 'string' ? option : option.name) === 'Create Department' ||
                option.isNew
            );
            if (!hasCreateOption) {
              filtered.push({
    ...
    if (type === 'date') {
      return (
        <DatePicker
          ...
          onChange={(newValue) =>
            handleInputChange(fieldname, newValue?.format('YYYY-MM-DD') || '')
          }
          slotProps={{
            textField: {
              fullWidth: true,
              ...
              {rows.length === 0 && (
                <TableRow>
                  <TableCell
                    colSpan={3}
                    align="center"
                    sx={{ py: 3, typography: 'body2', color: 'text.disabled' }}
                  >
                    No data available
                  </TableCell>
                </TableRow>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Box sx={{ p: 2 }}>
              ...
              <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr' }} gap={3}>
                ...
                <Box>{renderSalaryTable('Earning')}</Box>

                {/* Right Column - Deductions */}
                <Box>{renderSalaryTable('Deduction')}</Box>
              </Box>
                ...
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Box>
                    ...
                    >
                      <Box
                        component="span"
                        sx={{ fontFamily: "Arial, 'sans-serif'", mr: 1, fontSize: '0.8em' }}
                      >
                        {hrSettings.currency_symbol}
                      </Box>
                  ...
                  <Stack direction="row" spacing={4}>
                    <Box sx={{ textAlign: 'right' }}>
                      ...
                      >
                        +{' '}
                        <Box component="span" sx={{ fontFamily: "Arial, 'sans-serif'", mx: 0.5 }}>
                          {hrSettings.currency_symbol}
                        </Box>
                    ...
                    <Box sx={{ textAlign: 'right' }}>
                      ...
                      >
                        ...
                        {fNumber(totals.total_deductions || 0, {
