  ...
  return (
    ...
    >
      ...
      <DialogContent dividers sx={{ p: 4 }}>
        {loading ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Box>
              ...
              >
                ...
                <DetailCard
                  label="From Time"
                  value={wfh.from_time ? fTime(wfh.from_time) : '-'}
                  icon="solar:stopwatch-play-bold-duotone"
                />
                <DetailCard
                  label="To Time"
                  value={wfh.to_time ? fTime(wfh.to_time) : '-'}
                  icon="solar:stopwatch-pause-bold-duotone"
                />
                <DetailCard
                  label="Total Hours"
          ...
          <Box sx={{ py: 10, textAlign: 'center' }}>
            <Iconify
              icon={'solar:ghost-bold-duotone' as any}
              width={64}
      ...
      {isHR && wfh?.workflow_state === 'Pending' && (
        <DialogActions
          sx={{ px: 4, py: 3, bgcolor: (theme) => alpha(theme.palette.grey[500], 0.04) }}
        >
  ...
  return (
    <Box
      sx={{
        ...
        borderRadius: 2,
        bgcolor: (theme) =>
          highlight ? alpha(theme.palette.primary.main, 0.08) : 'background.neutral',
        border: (theme) =>
          highlight ? `1px solid ${alpha(theme.palette.primary.main, 0.2)}` : 'none',
        display: 'flex',
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Iconify
          icon={icon as any}
          width={20}
          sx={{ color: highlight ? 'primary.main' : 'text.disabled' }}
        />
        <Typography
          variant="caption"
