...
export default function CallDialog({ open, onClose, selectedCall, initialData, onSuccess }: Props) {
  ...
  const handleSaveCall = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to save call',
        severity: 'error',
      });
    }
  };
                    ...
                    {callData.call_for === 'Contact' && (
                      <Grid size={{ xs: 12, md: 6 }}>
                        <Autocomplete
                          ...
                          value={
                            contactOptions.find((opt) => opt.name === callData.contact_name) || null
                          }
                          onChange={(_, newValue) =>
                            setCallData({ ...callData, contact_name: newValue?.name || '' })
                          }
                          renderInput={(params) => (
                            <TextField
                              {...params}
                    ...
                    {callData.call_for === 'Accounts' && (
                      <Grid size={{ xs: 12, md: 6 }}>
                        <Autocomplete
                          ...
                          value={
                            accountOptions.find((opt) => opt.name === callData.account_name) || null
                          }
                          onChange={(_, newValue) =>
                            setCallData({ ...callData, account_name: newValue?.name || '' })
                          }
                          renderInput={(params) => (
                            <TextField
                              {...params}
                        ...
                        <FormControl fullWidth>
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setCallData({
                                ...callData,
                                completed_call_status: e.target.value as string,
                              })
                            }
                          >
                            {[
                  ...
                  <Grid size={{ xs: 12, md: 6 }}>
                    <DateTimePicker
                      ...
                      onChange={(newValue) =>
                        setCallData({
                          ...callData,
                          call_start_time: newValue ? newValue.format('YYYY-MM-DD HH:mm:ss') : '',
                        })
                      }
                    />
                  </Grid>
                  <Grid size={{ xs: 12, md: 6 }}>
                    <DateTimePicker
                      ...
                      onChange={(newValue) =>
                        setCallData({
                          ...callData,
                          call_end_time: newValue ? newValue.format('YYYY-MM-DD HH:mm:ss') : '',
                        })
                      }
                    />
                  </Grid>
                  ...
                  <FormControlLabel
                    control={
                      <Android12Switch
                        ...
                        onChange={(e) =>
                          setCallData({ ...callData, enable_reminder: e.target.checked ? 1 : 0 })
                        }
                      />
                    }
                  ...
                  {callData.enable_reminder === 1 && (
                    <Box>
                      <TimePicker
                        ...
                        value={dayjs()
                          .startOf('day')
                          .add(callData.remind_before_minutes || 60, 'minutes')}
