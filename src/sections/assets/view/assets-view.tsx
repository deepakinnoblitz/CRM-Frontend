...
export function AssetsView() {
  ...
  const { data, total, refetch } = useAssets(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleSortChange = (value: string) => {
    if (value === 'date_desc') {
      ...
      setOrderBy('purchase_cost');
      setOrder('desc');
    } else if (value === 'cost_asc') {
      setOrderBy('purchase_cost');
      setOrder('asc');
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `${selected.length} asset(s) deleted successfully`,
        severity: 'success',
      });
      setSelected([]);
      refetch();
    ...
    if (pendingFile) {
      ...
      try {
        ...
        if (fileUrl) {
          ...
          setSnackbar({
            open: true,
            message: 'File upload failed, proceeding without attachment',
            severity: 'error',
          });
        }
      } catch (error) {
  ...
  return (
    <DashboardContent maxWidth={false}>
      ...
      >
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1.5}>
            <Iconify
              icon={'solar:folder-plus-bold' as any}
              width={24}
            ...
            >
              ...
              <Autocomplete
                ...
                value={
                  ...
                  (category ? { name: category, category_name: category } : null)
                }
                ...
                renderOption={(props, option) => {
                  ...
                  return (
                    <li key={key} {...optionProps}>
                      ...
                      >
                        {option.isCreateNew && (
                          <Iconify icon="mingcute:add-line" sx={{ mr: 1, width: 16 }} />
                        )}
                  ...
                  {!assetAttachment && !pendingFile ? (
                    ...
                    >
                      ...
                      >
                        {pendingFile
                          ? pendingFile.name
                          : assetAttachment.split('/').pop() || assetAttachment}
                      </Typography>
                      <Button
                        size="small"
                        color="inherit"
      ...
      {/* View Dialog */}
      <AssetDetailsDialog open={openView} onClose={() => setOpenView(false)} asset={viewAsset} />
