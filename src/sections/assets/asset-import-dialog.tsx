...
export function AssetImportDialog({ open, onClose, onRefresh }: Props) {
  ...
  useEffect(() => {
    if (open) {
      getDocFields('Asset').then(setDocFields).catch(console.error);
    }
  }, [open]);
  ...
  const renderUpload = (
    <Box sx={{ py: 4, display: 'flex', flexFlow: 'column', alignItems: 'center' }}>
      ...
      <Button
        ...
        onClick={() => window.open(getTemplateUrl('Asset'), '_blank')}
        sx={{ mt: 2 }}
      >
