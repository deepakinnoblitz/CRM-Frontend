...
})<{ isToday?: boolean; statusColor?: string }>(({ theme, isToday, statusColor }) => ({
...
export function PremiumWorkingHours({
  ...
  const daysPresent = data.filter(
    (r) => (r.check_in && r.check_out) || (r.holiday_info && r.holiday_is_working_day === 0)
  ).length;
  ...
  const getStatusInfo = (record: AttendanceRecord) => {
    ...
    const color =
      colors[label] || (isNonWorking ? theme.palette.error.main : theme.palette.grey[500]);
    ...
    if (
      ...
      !record.check_in &&
      !record.check_out &&
      (status === 'Holiday' || status === 'Weekly Off' || status === 'Not Marked')
    ) {
      return {
        label: holiday || label,
        color,
  ...
  return (
    ...
    >
      <CardHeader
        title={title}
        subheader={
          data.length > 0 ? `${fDate(data[0].date)} - ${fDate(data[data.length - 1].date)}` : ''
        }
                    ...
                    <Box sx={{ width: '100%' }}>
                      ...
                      {!(isNonWorking && record.working_hours === 0) && (
                        <Stack
                          direction="row"
                          spacing={1}
                          sx={{ color: 'text.secondary', flexWrap: 'wrap', gap: 1.5 }}
                        >
                          <Stack
                            direction="row"
                            alignItems="center"
                          ...
                          >
                            <Iconify
                              icon={'solar:login-3-bold-duotone' as any}
                              width={14}
                          ...
                          >
                            <Iconify
                              icon={'solar:logout-3-bold-duotone' as any}
                              width={14}
                              sx={{ color: 'text.disabled', flexShrink: 0 }}
                            />
                            <Typography
