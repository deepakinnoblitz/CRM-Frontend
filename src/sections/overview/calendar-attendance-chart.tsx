...
export function CalendarAttendanceChart({
  title,
  subheader,
  calendarData,
  joiningDate,
  ...
  const breakdown = useMemo<AttendanceBreakdown>(() => {
    ...
    calendarData.forEach((record) => {
      ...
      if (record.status === 'Holiday' || (isSunday && !record.holiday_info)) {
        // Treat Sundays as holidays if not explicitly marked otherwise
        holiday += 1;
        return;
      }
  ...
  useEffect(() => {
    ...
    const labelItems = [
      ...
      { id: 'missing', label: 'Missing', value: breakdown.missing, color: statusColors.missing },
      {
        id: 'half_day',
        label: 'Half Day',
        value: breakdown.half_day,
        color: statusColors.half_day,
      },
    ].filter((item) => item.value > 0 && !(item.id === 'missing' && hideMissing));

    const total =
      breakdown.present + breakdown.absent + breakdown.missing + breakdown.half_day || 1;
    const cx = 250;
    const cy = 200;
    const radius = 105;
    const lineWidth = 30;
  ...
  const renderLabel = (item: any, top: number, side: 'left' | 'right') => (
    ...
    >
      ...
      <Stack direction="row" alignItems="baseline" spacing={0.3}>
        ...
        <Typography
          variant="caption"
          sx={{ color: 'text.disabled', fontWeight: 600, fontSize: '0.65rem' }}
        >
          ...
          <Stack spacing={3}>
            ...
            >
              {[
                ...
                .filter((kpi) => !(kpi.label === 'Missing' && hideMissing))
                .map((kpi) => (
