...
export function DashboardEomCard() {
  ...
  const sparkles = useMemo(() => {
    const generate = (count: number, minOffset: number, maxOffset: number) =>
      [...Array(count)].map(() => ({
  ...
  const monthName = new Date(award.month).toLocaleString('default', {
    month: 'long',
    year: 'numeric',
  });
  ...
  return (
    <Card
      sx={{
        ...
        boxShadow: (themeVar) =>
          `0 12px 24px -4px ${alpha('#D4AF37', 0.3)}, 0 4px 8px -2px ${alpha('#000', 0.05)}`,
        borderRadius: 2.5,
        mb: 3,
        minHeight: 180,
    ...
    >
      ...
      <Box sx={{ position: 'relative', flexGrow: 1, display: 'flex', alignItems: 'center' }}>
        ...
        <Stack
          spacing={0}
          sx={{ mt: { xs: 6, md: 5 }, pb: 3, width: '100%', maxWidth: { md: 280, xs: 200 } }}
        >
          <Typography
            variant="overline"
            sx={{
          ...
          <Typography
            variant="subtitle1"
            sx={{ color: 'text.secondary', fontWeight: 600, opacity: 0.8, lineHeight: 1.2 }}
          >
