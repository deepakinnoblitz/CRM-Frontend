...
export function JobOpeningsView() {
  ...
  const handleSortChange = (value: string) => {
    if (value === 'date_desc') {
      ...
      setOrderBy('job_title');
      setOrder('asc');
    } else if (value === 'title_desc') {
      setOrderBy('job_title');
      setOrder('desc');
    } else if (value === 'location_asc') {
      setOrderBy('location');
      setOrder('asc');
    } else if (value === 'location_desc') {
      setOrderBy('location');
      setOrder('desc');
    }
    setPage(0);
  ...
  return (
    <DashboardContent maxWidth={false}>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Stack spacing={3} sx={{ mt: 2 }}>
                  ...
                  const hasCreateOption = filtered.some(
                    (option: any) =>
                      (typeof option === 'string' ? option : option.name) ===
                        'Create Designation' || option.isNew
                  );
              ...
              <DatePicker
                ...
                onChange={(newValue) =>
                  setFormData({ ...formData, closes_on: newValue?.format('YYYY-MM-DD') || '' })
                }
                slotProps={{
                  textField: {
                    fullWidth: true,
      ...
      {/* View Dialog */}
      <JobOpeningDetailsDialog open={openView} onClose={() => setOpenView(false)} job={viewJob} />
