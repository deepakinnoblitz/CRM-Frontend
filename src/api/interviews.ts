...
export async function fetchInterviews(params: {
    ...
    const [res, countRes] = await Promise.all([
        ...
        frappeRequest(`/api/method/company.company.frontend_api.get_permitted_count?doctype=Interview&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`)
    ]);
