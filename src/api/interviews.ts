...
export async function fetchInterviews(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Interview&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getInterview(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Interview&name=${encodeURIComponent(name)}`
  );
...
export async function getInterviewPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Interview'
  );
