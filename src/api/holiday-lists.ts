...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'year desc, month desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Holiday List&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getHolidayList(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Holiday List&name=${encodeURIComponent(name)}`
  );
...
export async function getHolidayListPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Holiday List'
  );
...
export async function populateHolidays(month: string, year: string) {
  const res = await frappeRequest(
    `/api/method/company.company.api.populate_holidays_for_ui?month=${month}&year=${year}`
  );
