...
async function fetchFrappeList(params: {
  ...
  if (params.search) {
    filters.push([['Expenses', 'expense_category', 'like', `%${params.search}%`]]);
  }
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Expenses&filters=${encodeURIComponent(JSON.stringify(filters))}`
    ),
  ]);
...
export async function getExpensePermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Expenses'
  );
