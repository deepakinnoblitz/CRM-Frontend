...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'pay_period_start desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Salary Slip&filters=${encodeURIComponent(JSON.stringify(filters))}&_t=${Date.now()}`
    ),
  ]);
...
export async function submitSalarySlip(name: string) {
  const res = await frappeRequest(
    '/api/method/company.company.doctype.salary_slip.salary_slip.submit_salary_slip',
    {
      method: 'POST',
...
export async function getSalarySlip(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Salary Slip&name=${encodeURIComponent(name)}&_t=${Date.now()}`
  );
...
export async function getSalarySlipWithDetails(name: string) {
  const res = await frappeRequest(
    `/api/method/company.company.doctype.salary_slip.salary_slip.get_salary_slip_with_details?name=${encodeURIComponent(name)}&_t=${Date.now()}`
  );
...
export async function getSalarySlipPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Salary Slip'
  );
...
export async function previewSalarySlip(employee: string, start_date: string, end_date: string) {
  const res = await frappeRequest(
    '/api/method/company.company.doctype.salary_slip.salary_slip.preview_salary_slip',
    {
      method: 'POST',
...
export async function generateSalarySlipFromEmployee(
  employee: string,
  year: number,
  month: number,
  start_date?: string,
  ...
  const res = await frappeRequest(
    '/api/method/company.company.doctype.salary_slip.salary_slip.generate_salary_slips_from_employee',
    {
      method: 'POST',
      body: JSON.stringify({
        year,
...
export async function generateSalarySlipsForEmployees(
  year: number,
  month: number,
  employees: string[],
  start_date?: string,
