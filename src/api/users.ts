...
export async function fetchUsers(params: {
  ...
  try {
    const permRes = await frappeRequest(
      '/api/method/frappe.client.get_list?doctype=User Permission&fields=["user"]&limit_page_length=9999'
    );
    if (permRes.ok) {
      const permData = await permRes.json();
      usersWithPermissions = [
        ...new Set((permData.message || []).map((p: any) => p.user)),
      ] as string[];
    }
  } catch (error) {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=User&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
  ...
  const dataWithPermissions = users.map((user: any) => ({
    ...user,
    has_permission:
      usersWithPermissions.includes(user.name) || usersWithPermissions.includes(user.email),
  }));
...
export async function getUser(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=User&name=${encodeURIComponent(name)}`
  );
...
export async function fetchUserPermissions(user: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get_list?doctype=User Permission&filters=${JSON.stringify([['User Permission', 'user', '=', user]])}&fields=["*"]&limit_page_length=999`
  );
...
export async function getRoleProfiles() {
  const res = await frappeRequest(
    '/api/method/frappe.client.get_list?doctype=Role Profile&fields=["name"]&limit_page_length=999'
  );
...
export async function getRoles() {
  const res = await frappeRequest(
    '/api/method/frappe.client.get_list?doctype=Role&fields=["name"]&limit_page_length=999'
  );
...
export async function getModules() {
  const res = await frappeRequest(
    '/api/method/frappe.client.get_list?doctype=Module Def&fields=["name"]&limit_page_length=999'
  );
...
export async function changeUserPassword(userId: string, newPassword: string) {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.admin_change_user_password',
    {
      method: 'POST',
      headers: await getAuthHeaders(),
      body: JSON.stringify({
