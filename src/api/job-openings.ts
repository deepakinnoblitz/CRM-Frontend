...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'posted_on desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Job Opening&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getJobOpening(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Job Opening&name=${encodeURIComponent(name)}`
  );
...
export async function getJobOpeningPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Job Opening'
  );
...
export async function getJobLocations(): Promise<string[]> {
  try {
    const res = await frappeRequest(
      '/api/method/frappe.client.get_list?doctype=Job Opening&fields=["location"]&limit_page_length=999'
    );
    ...
    const uniqueLocations = [
      ...new Set(locations.map((item: any) => item.location).filter(Boolean)),
    ] as string[];
