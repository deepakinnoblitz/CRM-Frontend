...
export async function applyLeaveAllocationWorkflowAction(
  name: string,
  action: string,
  comment?: string
) {
...
export async function getLeaveAllocationWorkflowActions(
  currentState: string
...
export async function getLeaveAllocationPreview(
  year: number,
  month: number
): Promise<EmployeeAllocationPreview[]> {
  const headers = await getAuthHeaders();
