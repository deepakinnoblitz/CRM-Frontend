...
export async function fetchPurchaseCollections(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Purchase Collection&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getPurchaseCollection(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Purchase Collection&name=${encodeURIComponent(name)}`
  );
