...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'timesheet_date desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Timesheet&filters=${encodeURIComponent(JSON.stringify(filters))}`
    ),
  ]);
...
export async function getTimesheetPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Timesheet'
  );
...
export async function fetchProjects(params: { page: number; page_size: number; search?: string }) {
  const filters: any[] = [];
  if (params.search) {
