...
export async function fetchCRMExpenseTrackerStats(filters?: {
  start_date?: string;
  end_date?: string;
}) {
  ...
  const res = await frappeRequest(
    `/api/method/company.company.frontend_api.get_crm_expense_tracker_stats?${query.toString()}`
  );
...
export async function fetchCRMExpenseTrackerList(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=CRM Expense Tracker&filters=${encodeURIComponent(JSON.stringify(filters))}`
    ),
  ]);
