...
export async function fetchExpenseTrackerStats(filters?: {
  start_date?: string;
  end_date?: string;
}) {
  ...
  const res = await frappeRequest(
    `/api/method/company.company.frontend_api.get_expense_tracker_stats?${query.toString()}`
  );
  if (!res.ok) {
...
export async function fetchExpenseTrackerList(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Expense Tracker&filters=${encodeURIComponent(JSON.stringify(filters))}`
    ),
  ]);
