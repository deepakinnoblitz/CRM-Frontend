...
export async function fetchTaskManagerList(filters: any[] = []): Promise<TaskManager[]> {
  ...
  if (tasks.length > 0) {
    try {
      ...
      const assigneeRes = await frappeRequest(
        `/api/method/company.company.doctype.task_manager.task_manager.get_assignees?task_names=${JSON.stringify(taskNames)}`
      );
...
export async function getEmployeesFromDepartment(department: string): Promise<TaskAssignee[]> {
  const res = await frappeRequest(
    `/api/method/company.company.doctype.task_manager.task_manager.get_employees_from_department?department=${encodeURIComponent(department)}`
  );
  const data = await handleResponse(res);
  return data.message || [];
}
...
export async function getTaskManager(name: string): Promise<TaskManager> {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Task Manager&name=${name}`
  );
  const data = await handleResponse(res);
  return data.message;
}
...
export async function closeTaskManager(
  name: string,
  hours: string,
  remarks: string,
  attachment?: string
  ...
  const headers = await getAuthHeaders();
  const res = await frappeRequest(
    `/api/method/company.company.doctype.task_manager.task_manager.close_task`,
    {
      method: 'POST',
      headers,
      body: JSON.stringify({
...
export async function reopenTaskManager(name: string, remarks: string): Promise<void> {
  const headers = await getAuthHeaders();
  const res = await frappeRequest(
    `/api/method/company.company.doctype.task_manager.task_manager.reopen_task`,
    {
      method: 'POST',
      headers,
...
export async function acceptTaskManager(name: string): Promise<void> {
  const headers = await getAuthHeaders();
  const res = await frappeRequest(
    `/api/method/company.company.doctype.task_manager.task_manager.accept_task`,
    {
      method: 'POST',
      headers,
...
export async function fetchDepartments(
  search?: string
): Promise<{ name: string; department_name: string }[]> {
  const filters: any[] = [];
  if (search) {
 ...
 */
export async function fetchAllActiveEmployees(): Promise<
  { name: string; employee_name: string }[]
> {
  const res = await frappeRequest(
    `/api/method/company.company.doctype.task_manager.task_manager.get_all_active_employees`
  );
  const data = await handleResponse(res);
  return data.message || [];
}

export async function getTaskManagerPermissions(): Promise<{
  read: boolean;
  write: boolean;
  create: boolean;
  delete: boolean;
  ...
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Task Manager'
  );
