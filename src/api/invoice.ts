...
export async function fetchInvoices(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Invoice&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getInvoice(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Invoice&name=${encodeURIComponent(name)}`
  );
...
export async function getInvoicePermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Invoice'
  );
