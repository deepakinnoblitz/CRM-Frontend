...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'modified desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Asset Assignment&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getAssetAssignment(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Asset Assignment&name=${name}`
  );
...
export async function getAssetAssignmentPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Asset Assignment'
  );
...
export async function getEmployees(): Promise<Array<{ name: string; employee_name: string }>> {
  try {
    const res = await frappeRequest(
      '/api/method/frappe.client.get_list?doctype=Employee&fields=["name","employee_name"]&limit_page_length=999'
    );
...
export async function getAvailableAssets(): Promise<Array<{ name: string; asset_name: string }>> {
  try {
    const res = await frappeRequest(
      '/api/method/company.company.frontend_api.get_available_assets_list'
    );
...
export async function getMyAssignedAssets(
  employee?: string
): Promise<Array<{ asset: string; asset_name: string }>> {
  try {
    const query = employee ? `?employee=${employee}` : '';
    const res = await frappeRequest(
      `/api/method/company.company.frontend_api.get_my_assigned_assets${query}`
    );
