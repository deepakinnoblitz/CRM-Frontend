...
export async function fetchEstimations(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Estimation&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getEstimation(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Estimation&name=${encodeURIComponent(name)}`
  );
...
export async function getEstimationPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Estimation'
  );
...
export async function convertEstimationToInvoice(name: string) {
  ...
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.convert_estimation_to_invoice',
    {
      method: 'POST',
      headers,
