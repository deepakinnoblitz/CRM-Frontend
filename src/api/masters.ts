...
export async function getDepartment(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Department&name=${encodeURIComponent(name)}`
  );
...
export async function getProject(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Project&name=${encodeURIComponent(name)}`
  );
...
export async function getActivityType(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Activity Type&name=${encodeURIComponent(name)}`
  );
...
export async function getBankAccount(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Bank Account&name=${encodeURIComponent(name)}`
  );
...
export const fetchClaimTypes = (params: any) => {
  ...
  const or_filters = search ? [['Claim Type', 'claim_type', 'like', `%${search}%`]] : undefined;
...
export async function getClaimType(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Claim Type&name=${encodeURIComponent(name)}`
  );
...
export async function getAssetCategory(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Asset Category&name=${encodeURIComponent(name)}`
  );
...
export async function createEvaluationTraitCategory(data: Partial<EvaluationTraitCategory>) {
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to create evaluation trait category'));
  return json.message;
}

export async function updateEvaluationTraitCategory(
  name: string,
  data: Partial<EvaluationTraitCategory>
) {
  const headers = await getAuthHeaders();
  const res = await frappeRequest('/api/method/frappe.client.set_value', {
    method: 'POST',
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to update evaluation trait category'));
  return json.message;
}
...
export async function deleteEvaluationTraitCategory(name: string) {
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to delete evaluation trait category'));
  return true;
}
...
export async function renameEvaluationTraitCategory(oldName: string, newName: string) {
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to rename evaluation trait category'));
  return json.message;
}

export async function getEvaluationTraitCategory(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Evaluation Trait Category&name=${encodeURIComponent(name)}`
  );
...
export async function getDesignation(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Designation&name=${encodeURIComponent(name)}`
  );
...
export async function createSalaryStructureComponent(data: Partial<SalaryStructureComponent>) {
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to create salary structure component'));
  return json.message;
}

export async function updateSalaryStructureComponent(
  name: string,
  data: Partial<SalaryStructureComponent>
) {
  const headers = await getAuthHeaders();
  const res = await frappeRequest('/api/method/frappe.client.set_value', {
    method: 'POST',
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to update salary structure component'));
  return json.message;
}
...
export async function deleteSalaryStructureComponent(name: string) {
  ...
  const json = await res.json();
  if (!res.ok)
    throw new Error(handleFrappeError(json, 'Failed to delete salary structure component'));
  return true;
}

export async function getSalaryStructureComponent(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Salary Structure Component&name=${encodeURIComponent(name)}`
  );
