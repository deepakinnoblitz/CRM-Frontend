...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order
      ? `${params.orderBy} ${params.order}, creation ${params.order}`
      : 'date_of_expense desc, creation desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Reimbursement Claim&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getClaimTypes() {
  const res = await frappeRequest(
    `/api/method/company.company.frontend_api.get_doctype_list?doctype=Claim Type&fields=["*"]`
  );
  if (!res.ok) return [];
  const data = await res.json();
  return data.message || [];
...
export async function getReimbursementClaim(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Reimbursement Claim&name=${encodeURIComponent(name)}`
  );
...
export async function getReimbursementClaimPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Reimbursement Claim'
  );
...
export async function getReimbursementClaimWorkflowActions(
  currentState: string
...
export async function applyReimbursementClaimWorkflowAction(
  name: string,
  action: string,
  comment?: string,
  paymentDetails?: any
  ...
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.apply_workflow_action',
    {
      method: 'POST',
      headers,
      body: JSON.stringify({
