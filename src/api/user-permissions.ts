...
export async function fetchUserPermissions(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=User Permission&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getDocTypes() {
  const res = await frappeRequest(
    '/api/method/frappe.client.get_list?doctype=DocType&fields=["name"]&filters=[["DocType","issingle","=",0],["DocType","istable","=",0]]&limit_page_length=999&order_by=name asc'
  );
...
export async function getUsers() {
  const res = await frappeRequest(
    '/api/method/frappe.client.get_list?doctype=User&fields=["name","email","full_name"]&filters=[["User","enabled","=",1]]&limit_page_length=999&order_by=full_name asc'
  );
