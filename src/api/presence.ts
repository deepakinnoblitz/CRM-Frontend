...
export async function checkTodayTimesheet(employee?: string) {
  const res = await frappeRequest(
    '/api/method/company.company.presence_api.check_today_timesheet',
    {
      method: 'POST',
      body: JSON.stringify({ employee }),
    }
...
export async function getPresenceSettings() {
  const res = await frappeRequest(
    '/api/method/company.company.doctype.employee_presence_settings.employee_presence_settings.get_presence_settings',
    {
      method: 'POST',
      body: JSON.stringify({}),
    }
...
export async function updatePresenceSettings(settings: {
  ...
  const res = await frappeRequest(
    '/api/method/company.company.doctype.employee_presence_settings.employee_presence_settings.set_presence_settings',
    {
      method: 'POST',
