...
export async function fetchLeads(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Lead&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getDoctypeList(
  doctype: string,
  fields?: string[],
...
export async function getWorkflowActions(
  doctype: string = 'Lead',
  currentState: string
): Promise<{ action: string; next_state: string }[]> {
  const res = await frappeRequest(
...
export function getNextStates(currentState: string, transitions: WorkflowTransition[]): string[] {
  ...
  const nextStates = transitions.filter((t) => t.state === currentState).map((t) => t.next_state);
