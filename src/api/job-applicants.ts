...
export async function fetchJobApplicants(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Job Applicant&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getJobApplicant(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Job Applicant&name=${encodeURIComponent(name)}`
  );
...
export async function getJobApplicantPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Job Applicant'
  );
