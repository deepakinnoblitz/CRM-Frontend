...
export async function checkLeaveBalance(params: {
  ...
  const res = await frappeRequest(
    `/api/method/company.company.api.check_leave_balance?${query.toString()}`
  );
  if (!res.ok) {
...
export async function getEmployeeProbationInfo(employee: string, date?: string) {
  ...
  const res = await frappeRequest(
    `/api/method/company.company.api.get_employee_probation_info?${query.toString()}`
  );
  if (!res.ok) {
    const error = await res.json();
...
export async function applyLeaveWorkflowAction(
  name: string,
  action: string,
  comment?: string,
  updateData?: any
