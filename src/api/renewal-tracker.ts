...
async function fetchFrappeList(params: {
  ...
  const orderByParam =
    params.orderBy && params.order ? `${params.orderBy} ${params.order}` : 'renewal_date desc';
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Renewal Tracker&filters=${encodeURIComponent(JSON.stringify(filters))}`
    ),
  ]);
...
export async function getRenewal(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Renewal Tracker&name=${encodeURIComponent(name)}`
  );
...
export async function getRenewalPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Renewal Tracker'
  );
