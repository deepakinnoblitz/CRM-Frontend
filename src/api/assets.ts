...
async function fetchFrappeList(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Asset&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getAssetPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Asset'
  );
...
export async function getAssetCategories(): Promise<any[]> {
  try {
    const res = await frappeRequest(
      '/api/method/frappe.client.get_list?doctype=Asset Category&fields=["name","category_name"]&limit_page_length=999'
    );

    if (!res.ok) {
      // Fallback to distinct list from Asset if Asset Category doesn't exist or fails
      const assetRes = await frappeRequest(
        '/api/method/frappe.client.get_list?doctype=Asset&fields=["category"]&distinct=true'
      );
