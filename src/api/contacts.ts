...
export async function fetchContacts(params: {
  ...
  const [res, countRes] = await Promise.all([
    ...
    frappeRequest(
      `/api/method/frappe.client.get_count?doctype=Contacts&filters=${encodeURIComponent(JSON.stringify(filters))}&or_filters=${encodeURIComponent(JSON.stringify(or_filters))}`
    ),
  ]);
...
export async function getContactPermissions() {
  const res = await frappeRequest(
    '/api/method/company.company.frontend_api.get_doc_permissions?doctype=Contacts'
  );
...
export async function getContact(name: string) {
  const res = await frappeRequest(
    `/api/method/frappe.client.get?doctype=Contacts&name=${encodeURIComponent(name)}`
  );
